...
<div class="mx-auto max-w-3xl overflow-hidden rounded-lg bg-white shadow-sm ring-1 ring-slate-200">
    <div class="border-b border-slate-200 px-6 py-4">
        <h1 class="text-lg font-semibold text-slate-900">Edit {{ $title }}</h1>
    </div>
    ...
    <div class="px-6 py-6">
        ...
        <form action="{{ route("$route.update", $entity->id) }}" method="POST">
            ...
            <div class="mt-6 flex flex-wrap items-center gap-3 border-t border-slate-200 pt-6">
                ...
                <a href="{{ route("$route.show", $entity) }}"
                   class="inline-flex items-center gap-1.5 rounded-md bg-amber-50 px-4 py-2 text-sm font-medium text-amber-700 ring-1 ring-inset ring-amber-200 transition hover:bg-amber-100">
                    Cancel
                </a>
